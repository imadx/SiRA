...
public class callReceiver extends BroadcastReceiver {
    ...
    private static final String TAG = "CstomPhoneStateListener";
        ...
        telephony.listen(new PhoneStateListener() {
            ...
            public void onCallStateChanged(int state, String incomingNumber) {
                ...
                if (telephony.CALL_STATE_RINGING == state) {
                    ...
                    String res="I'm currently engaged in "+response;
                    if(!response.equals("")){
                       Log.i(TAG,"something");
                        if(ifSiRATurnedOn==1){
                            MainActivity.manager.setRingerMode(AudioManager.RINGER_MODE_SILENT);
                            SmsReceiver.sendSMSMessage(incomingNumber, res, context);
                        }
                        //setResultData(null);
                    }else{
                        Log.i(TAG, "");
